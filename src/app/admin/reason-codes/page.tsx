...
export default async function ReasonCodesPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 px-4 py-9">
      <PageHeader
        title="Reason codes"
        subtitle="These are the options people pick from on the floor. Keeping them a fixed list is what lets you count them later and find the real repeat offenders."
        actions={
          <Link
            href="/admin"
        ...
        {grouped.map((g) => (
          <section key={g.key}>
            ...
            <Panel className="divide-y divide-steel-100">
              {g.rows.length === 0 && (
                <p className="px-5 py-4 text-sm text-steel-400">Nothing set up yet.</p>
              )}
              {g.rows.map((c) => (
                <div key={c.id} className="flex items-center justify-between gap-3 px-5 py-3">
                  <div>
                    <p className={`text-sm ${c.active ? "text-steel-900" : "text-steel-400 line-through"}`}>
                      {c.label}
                    </p>
                    <p className="text-xs text-steel-400 tnum">{c.code}</p>
                  </div>
      ...
      <Panel className="mt-8 p-5">
        <p className="mb-3 text-sm font-medium text-steel-700">Add a reason</p>
        <NewReasonCodeForm />
      </Panel>
    </div>
  );
