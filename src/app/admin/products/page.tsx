  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 px-4 py-9">
      <PageHeader
        ...
        subtitle="Anything made here can have its own list of steps. Anything bought in is simply consumed from stock."
        actions={
          <Link
            href="/admin"
        ...
        <Panel className="divide-y divide-steel-100">
          {made.map((item) => (
            <Link
              ...
              className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 hover:bg-steel-50"
            >
              <div>
                <p className="text-sm font-medium text-steel-900">
                  {item.name}
                  {item.isFinishedGood && (
                    <span className="ml-2 rounded bg-chartreuse/25 px-1.5 py-0.5 text-xs text-navy-800">
                      sold product
                    </span>
                  )}
                </p>
                <p className="text-xs text-steel-400 tnum">{item.sku}</p>
              </div>
        ...
        <Panel className="divide-y divide-steel-100">
          {bought.map((item) => (
            <div key={item.id} className="flex items-center justify-between gap-3 px-5 py-3">
              <div>
                <p className="text-sm text-steel-900">{item.name}</p>
                <p className="text-xs text-steel-400 tnum">{item.sku}</p>
              </div>
              <p className="text-xs text-steel-400">per {item.unitOfMeasure}</p>
            </div>
          ))}
        </Panel>
      </section>
