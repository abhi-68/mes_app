# Source: tests/task-action.test.mts

Integration tests for the application action — successful issue, retry, shortage,
held stock, unauthorized access and transactional rollback. Commit 6196158.
...
test("successful start: one motor issued, task started, clock running, event written", async () => {
  ...
  assert.equal((await balance()).onHand, 2, "exactly one motor consumed");
  assert.equal((await balance()).activeReserved, 0, "reservation relieved by the issue");
...
test("duplicate request with the SAME command id consumes material once", async () => {
  await receiveStock({ commandId: uid("rcv"), itemId: MOTOR, locationId: STORES, quantity: 3 });
  ...
  assert.equal((await balance()).onHand, 2, "ONE motor consumed, not two");

  const issues = await db
    .select()
    .from(inventoryMovements)
    .where(eq(inventoryMovements.type, "ISSUE"));
  assert.equal(issues.length, 1, "exactly one issue movement");
});

test("a NEW command id does not let the same start consume twice", async () => {
  await receiveStock({ commandId: uid("rcv"), itemId: MOTOR, locationId: STORES, quantity: 3 });

  await startTask(TASK, "cmd-first");
  // A different id is a different command, but the requirement is already satisfied,
  // so coverage short-circuits and nothing further is drawn.
  await startTask(TASK, "cmd-second");

  assert.equal((await balance()).onHand, 2, "still only one motor consumed");
  const issues = await db
    .select()
    .from(inventoryMovements)
    .where(eq(inventoryMovements.type, "ISSUE"));
