...
async function clearPreviousRuns() {
  const tasks = `SELECT t.id FROM work_order_tasks t JOIN work_orders w ON w.id = t.work_order_id
                  WHERE w.order_number LIKE 'SIM-%'`;
  const orders = `SELECT id FROM work_orders WHERE order_number LIKE 'SIM-%'`;
  ...
  await q(`DELETE FROM disposition_records WHERE operation_id IN (${tasks})`);
  ...
  await q(`DELETE FROM operation_dependencies WHERE operation_id IN (${tasks})
             OR depends_on_operation_id IN (${tasks})`);
  await q(`DELETE FROM reservations WHERE requirement_id IN
             (SELECT id FROM material_requirements WHERE operation_id IN (${tasks}))`);
  await q(`DELETE FROM material_requirements WHERE operation_id IN (${tasks})`);
  ...
  await q(`DELETE FROM work_order_tasks WHERE work_order_id IN (${orders})`);
  await q(`DELETE FROM work_orders WHERE order_number LIKE 'SIM-%'`);
  await q(`DELETE FROM inventory_holds WHERE item_id IN (${simItems})`);
  await q(`DELETE FROM inventory_movements WHERE item_id IN (${simItems})`);
...
/**
 * The card for a named step on /my-station, scoped to THIS run's order — the same
 * step name exists on every previous simulation's order, and picking the first one
 * on the page silently drives someone else's job.
 */
const cardFor = (name) =>
...
async function finishStep(name) {
  await page.goto(`${BASE}/my-station`);
  await page.waitForTimeout(700);
  await act_(cardFor(name).getByRole("button", { name: "Mark done" }).first(), 1800);
}
...
check(
  "starting the step drew exactly the two blades the BOM calls for",
  Number(before.on_hand) - Number(afterStart.on_hand) === 2,
  `${before.on_hand} → ${afterStart.on_hand}`
);
check(
  "one issue movement, not two",
...
check(
  ...
  (await fitStart.count()) > 0 && (await fitStart.isDisabled()),
  await fitCard.locator("span").filter({ hasText: "Earlier step" }).count() > 0
    ? "and the card says which step"
    : ""
);
...
await act_(blockCard.getByRole("button", { name: "Can't continue" }).first(), 600);
await blockCard
  .getByPlaceholder("e.g. 60 ft of copper tube outstanding")
  .fill("Damper bolts are the wrong thread pitch");
await act_(blockCard.getByRole("button", { name: "Flag as blocked" }).first(), 1600);

const blocked = await taskRow("Fit damper section");
check("the step is blocked", blocked.status === "BLOCKED", blocked.status);
check("with the worker's own words kept", /thread pitch/.test(blocked.blocked_note ?? ""), blocked.blocked_note);
...
check(
  ...
  (await page.getByText("thread pitch").count()) > 0
);
...
await page.goto(`${BASE}/my-station`);
await page.waitForTimeout(900);
await act_(cardFor("Fit damper section").getByRole("button", { name: "Unblock" }).first(), 1600);
...
// By value, not by label: selectOption does not accept a pattern, and the option
// text carries the station name after the person's.
const jordan = await one(`SELECT id FROM users WHERE email = 'worker2@thermal-corp.com'`);
await handCard.locator("select").last().selectOption(String(jordan.id));
await act_(handCard.getByRole("button", { name: /Give it to them/ }).first(), 1800);
...
if (openNow.length >= 2) {
  ...
  // Close them both and read the books.
  for (const name of [secondReady?.name, "Cut damper blades"].filter(Boolean)) {
    await page.goto(`${BASE}/my-station`);
    await page.waitForTimeout(800);
    const c = page.locator(`[data-step-card="${name}"]`).filter({ hasText: /SIM-/ }).first();
    const off = c.getByRole("button", { name: "Clock off" }).first();
    if ((await off.count()) > 0) await act_(off, 1500);
  }
  ...
  const entries = await q(
    `SELECT te.started_at, te.ended_at, te.duration_seconds
      ...
      ORDER BY te.started_at DESC LIMIT 2`
  );
  const overlapping =
    entries.length === 2 &&
    new Date(entries[0].started_at) < new Date(entries[1].ended_at) &&
    new Date(entries[1].started_at) < new Date(entries[0].ended_at);
  check("the two sessions really did overlap", overlapping, "otherwise the next check proves nothing");
